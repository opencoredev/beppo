...
const STARTUP_REDIRECT_TIMEOUT_MS = 15_000;
  ...
  <body>
    <main class="card" aria-live="polite">
      ...
      <div class="row">
        ...
        <span id="startup-status">Waiting for the local server to respond...</span>
      </div>
      <p id="startup-help">This should only take a moment.</p>
    </main>
    <script>
      (() => {
        const targetUrl = ${JSON.stringify(targetUrl)};
        const backendWsUrl = ${JSON.stringify(backendWsUrl)};
        const timeoutMs = ${STARTUP_REDIRECT_TIMEOUT_MS};
        const startedAt = Date.now();
        const statusEl = document.getElementById("startup-status");
        const helpEl = document.getElementById("startup-help");
        let redirected = false;
        let retryTimer = null;

        const setTimedOutState = () => {
          if (statusEl) {
            statusEl.textContent = "Beppo is taking longer than expected to start.";
          }
          if (helpEl) {
            helpEl.textContent =
              "Try restarting the app. If this keeps happening, check the desktop logs for backend startup failures.";
          }
        };
