...
if (isBunRuntime) {
  it.skip("NodeSqliteClient requires Node's built-in sqlite module", () => {});
} else {
  it.effect("NodeSqliteClient runs prepared queries and returns positional values", () =>
    Effect.tryPromise(() => import("./NodeSqliteClient.ts")).pipe(
      Effect.flatMap((SqliteClient) =>
        Effect.gen(function* () {
